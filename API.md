...
- 时间字段：ISO 8601 字符串（建议带时区，如 `2026-09-10T23:00:00Z`，也接受 `YYYY-MM-DD`、空格分隔与 `+08:00` 偏移），或正整数毫秒时间戳；读数时间戳按位置**严格递增**
- **公历日期严格校验**：不存在的日期一律 400 拒绝且不写库，不做静默进位。例如 `2026-02-31`、非闰年 `2026-02-29`、`2026-13-01`、`2026-09-10T25:00:00Z` 均拒绝（不会把 `02-31` 滚成 `03-03`）；闰年 `2024-02-29` 等合法边界日期正常接受
...
| 参数 | 说明 |
|---|---|
| rubbingId | **只返回读数时间落在该拓片保存时段 `[startAt,endAt)` 内的记录**（同位置时段之前/之后的读数不计入；半开区间，含 `startAt` 整点、不含 `endAt` 整点）。同一位置被多个拓片复用时，按各拓片自己的时段裁剪；开放保存时段其后的读数持续计入 |
| locationId | 不限拓片时返回该位置全部读数；与 rubbingId 同时给出时，该位置须属于该拓片的保存时段，否则为空，且仍按其保存时段裁剪 |
| breached | `true` 仅越限读数，`false` 仅正常读数 |
| from / to | 时间范围（含端点，ISO 8601 或毫秒）；参数本身为非法公历日期时返回 400 |
...
> 注意：按拓片查询是“事件时间落在保存时段内”的严格裁剪，而不是“该位置涉及过就全量返回”。
> 若某库位先存放拓片 A、空档期发生异常、再存放拓片 B，则 A、B 按拓片查询都看不到该空档异常；
> 不带 `rubbingId`、仅按 `locationId` 查询可看到该位置的完整事件流。
